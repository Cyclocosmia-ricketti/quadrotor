...
double pwmout1, pwmout2, pwmout3, pwmout4;
...
void TIM3_Cap_Init(u16 arr, u16 psc)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //PB6,7,8,9 输入 
	GPIO_Init(GPIOA, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOA, GPIO_Pin_6 | GPIO_Pin_7 );//PB6,7,8,9  下拉
	
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 ;  //PB6,7,8,9 清除之前设置  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //PB6,7,8,9 输入 
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOB, GPIO_Pin_0 | GPIO_Pin_1 );//PB6,7,8,9  下拉
...
double Read4(void)
{
	pwmout4=(100.0*tempup4/period-7.5)*4.0;
	return pwmout4;
}
